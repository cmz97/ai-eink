...
class GUI:
    def __init__(self, eink_width=240, eink_height=416, FONT_PATH='./Asset/Font/Monorama-Bold.ttf', contents = [
        ...
        for value in contents:
            itemBox, mask = self.draw_rounded_rectangle_with_mask(eink_width, 90, 10, (10, 10, 10, 10), 3, fill=False)
            self.masks.append(mask)
            mask.save('mask.bmp')

            itemBox = self.create_row_layout(
                itemBox,
                value["Icon"],
                value["Title"],
                FONT_PATH,
            ...
            top_left = (0, 45 + counter * 90)
            # Paste the item box
            self.canvas.paste(itemBox, top_left)
            self.maskPosition.append(top_left)
    ...
    def updateIndex(self, index, previousIndex):
        ...
        self.canvas = self.invert_pixels_within_region(self.canvas, self.masks[index], self.maskPosition[index])
        self.canvas.save('result.bmp')
    ...
    @jit(nopython=True)
    def _dithering_1_bit(self,pixels:np.ndarray):
        """
        Apply Floyd-Steinberg dithering to convert an array of grayscale pixels to 1-bit.
    ...
    @jit(nopython=True)
    def _dithering_2_bit(self,pixels:np.ndarray):
    ...
    def apply_dithering(self,image:Image, bits:int):
        ...
        if bits == 1:
            dithered_pixels = self._dithering_1_bit(pixels)
        elif bits == 2:
            dithered_pixels = self._dithering_2_bit(pixels)

        # Clip the values and convert back to an image
        dithered_pixels = np.clip(dithered_pixels, 0, 255)
        return Image.fromarray(dithered_pixels.astype(np.uint8))


    def apply_2bit_quant(self,image):
